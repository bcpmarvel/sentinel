...
@app.command()
def main(
    source: Annotated[Optional[str], typer.Option("--source", "-s", help="Video source (webcam index or path)")] = None,
    conf: Annotated[Optional[float], typer.Option("--conf", "-c", min=0.0, max=1.0, help="Confidence threshold")] = None,
    device: Annotated[Optional[Device], typer.Option("--device", "-d", help="Device for inference")] = None,
    track: Annotated[bool, typer.Option("--track", "-t", help="Enable tracking")] = False,
    model: Annotated[Optional[str], typer.Option("--model", "-m", help="Path to YOLO model")] = None,
    analytics: Annotated[bool, typer.Option("--analytics", "-a", help="Enable zone analytics")] = False,
    zones: Annotated[Optional[str], typer.Option("--zones", "-z", help="Path to zones JSON")] = None,
    config: Annotated[Optional[str], typer.Option("--config", help="Load settings from TOML file")] = None,
